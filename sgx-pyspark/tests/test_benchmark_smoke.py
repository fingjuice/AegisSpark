...
def tiny_env(tmp_path, monkeypatch):
    ...
    if keys_src.is_dir():
        ...
        ecdsa = ECDSACrypto()
        ecdsa.generate_keypair(keys_dst / "admin_ecdsa.pem", keys_dst / "admin_ecdsa.pub")
        ecdsa.generate_keypair(keys_dst / "driver_tee_ecdsa.pem", keys_dst / "driver_tee_ecdsa.pub")
    return tmp_path
