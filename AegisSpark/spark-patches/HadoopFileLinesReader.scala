...
class HadoopFileLinesReader(
  ...
  private val _iterator = {
    if (org.apache.spark.abe.AbeConf.isEnabled(conf)) {
      val sep: Array[Byte] = lineSeparator.orNull
      new org.apache.spark.abe.AbeFileLinesReader(
        file.filePath, file.start, file.length, sep, conf)
    } else {
