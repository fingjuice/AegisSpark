...
class SparkContext(config: SparkConf) extends Logging {
  ...
  try {
    ...
    _conf.getOption(org.apache.spark.abe.AbeConf.ABE_MASTER_CONFIG).foreach { masterPath =>
      try {
        val settings = org.apache.spark.abe.AbeSparkConfigLoader.load(masterPath)
        org.apache.spark.abe.AbeSparkConfigLoader.applyToHadoopIfAbsent(_hadoopConfiguration, settings)
        if (!_conf.contains(org.apache.spark.abe.AbeConf.ABE_ENABLED)) {
          _conf.set(org.apache.spark.abe.AbeConf.ABE_ENABLED, settings.enabled.toString)
        }
        if (!_conf.contains(org.apache.spark.abe.AbeConf.ABE_CRYPTO_CONFIG)) {
          _conf.set(org.apache.spark.abe.AbeConf.ABE_CRYPTO_CONFIG, masterPath)
        }
        if (!_conf.contains(org.apache.spark.abe.AbeConf.ABE_USER_ID) && settings.userId != null) {
          _conf.set(org.apache.spark.abe.AbeConf.ABE_USER_ID, settings.userId)
        }
        if (!_conf.contains(org.apache.spark.abe.AbeConf.ABE_USER_ATTRIBUTES) &&
            settings.userAttributes != null) {
          _conf.set(org.apache.spark.abe.AbeConf.ABE_USER_ATTRIBUTES, settings.userAttributes)
        }
        if (!_conf.contains(org.apache.spark.abe.AbeConf.ABE_WRITE_VERIFICATION_URL) &&
            settings.writeVerificationUrl != null) {
          _conf.set(org.apache.spark.abe.AbeConf.ABE_WRITE_VERIFICATION_URL, settings.writeVerificationUrl)
        }
      } catch {
        case e: java.io.IOException =>
          logWarning(s"Failed to load ABE master config: $masterPath", e)
      }
    }
    org.apache.spark.abe.AbeWriteClient.propagateAbeSettings(
      _hadoopConfiguration,
      _conf.getBoolean(org.apache.spark.abe.AbeConf.ABE_ENABLED, false),
      _conf.get(org.apache.spark.abe.AbeConf.ABE_CRYPTO_CONFIG, ""),
      _conf.get(org.apache.spark.abe.AbeConf.ABE_USER_ID, ""),
      _conf.get(org.apache.spark.abe.AbeConf.ABE_USER_ATTRIBUTES, ""))
