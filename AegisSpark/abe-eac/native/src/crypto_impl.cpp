...
class ECDSACryptoImpl : public IECDSACrypto {
public:
	SignResult sign(const std::vector<uint8_t>& message, const std::string& private_key_pem_path)
	{
		...
		if (!pkey) {
			out.ok = false;
			out.reason = "failed to load admin private key: " + private_key_pem_path;
			return out;
		}
		...
		if (!mdctx ||
			EVP_DigestSignInit(mdctx, NULL, EVP_sha256(), NULL, pkey) != 1 ||
			EVP_DigestSignUpdate(mdctx, message.data(), message.size()) != 1 ||
			EVP_DigestSignFinal(mdctx, NULL, &sigLen) != 1) {
			EVP_MD_CTX_free(mdctx);
			EVP_PKEY_free(pkey);
			out.ok = false;
			out.reason = "ECDSA sign init failed";
			return out;
		}
		sig.resize(sigLen);
		if (EVP_DigestSignFinal(mdctx, sig.data(), &sigLen) != 1) {
			...
			EVP_PKEY_free(pkey);
			out.ok = false;
			out.reason = "ECDSA sign final failed";
			return out;
		}
		sig.resize(sigLen);
		EVP_MD_CTX_free(mdctx);
		...
		const std::string& public_key_pem_path)
	{
		...
		if (!mdctx ||
			...
			EVP_PKEY_free(pkey);
			out.ok = false;
			out.reason = "ECDSA verify init failed";
			return out;
		}
		...
		EVP_PKEY_free(pkey);
		out.ok = (rc == 1);
		if (!out.ok) out.reason = "ECDSA signature verification failed";
		return out;
	}
};
	...
	: config_(config)
{
	...
	ecdsa_.reset(new ECDSACryptoImpl());
