...
public final class CompanyAbeBenchmark {
  ...
  private static Iterator<TableStats> writePartition(BenchContext ctx, Iterator<Long> ids) {
    ...
    while (ids.hasNext()) {
      ...
      try {
        ...
        String sig = AbeNativeBridge.signEd25519(tp, ctx.driverPriv);
  ...
  static final class BenchContext implements Serializable {
    ...
    static BenchContext fromIni(String configPath, Map<String, String> ini) {
      ...
      c.adminPub = confDir + "/admin_ed25519.pub";
      c.driverPub = confDir + "/driver_tee_ed25519.pub";
      c.driverPriv = confDir + "/driver_tee_ed25519.pem";
      c.eacPub = confDir + "/write_verification_ed25519.pub";
      c.eacPriv = confDir + "/write_verification_ed25519.pem";
  ...
  private static AdminEndorsement buildEndorsement(BenchContext ctx) {
    ...
    String adminPriv = ctx.driverPriv.replace("driver_tee_ed25519.pem", "admin_ed25519.pem");
    String sig = AbeNativeBridge.signEd25519(payload, adminPriv);
