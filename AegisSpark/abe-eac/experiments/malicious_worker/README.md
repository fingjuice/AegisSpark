...
|----------------------|-----------------|
| TaskTicket / Driver Ed25519 | `TaskTicket.java`, `DriverTeeOrchestrator` |
...
|----------|-------------|--------------------|
| **A** Ticket tampering | Mutate `allowed_target_path` / `task_id` / `expires_at`, or write `/finance/salary` | Ed25519 verify fail or path-scope check |
...
abe-eac/experiments/malicious_worker/
  aegis_defense_sim.py              # defense semantics (Ed25519 + policy engine)
...
2. Intercepts happen on the local TEE/EAC path; latency is typically **hundreds of µs to sub-ms** (Ed25519 verify dominated).
