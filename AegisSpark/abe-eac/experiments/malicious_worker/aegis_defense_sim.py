...
# ---------------------------------------------------------------------------
# Crypto helpers (ECDSA-P256-SHA256, same curve as abe-spark.conf)
# ---------------------------------------------------------------------------
...
class EcdsaKeyPair:
    ...
    def sign(self, message: bytes) -> bytes:
        return self._sk.sign(message, ec.ECDSA(hashes.SHA256()))

    def verify(self, message: bytes, signature: bytes) -> bool:
        try:
            self._vk.verify(signature, message, ec.ECDSA(hashes.SHA256()))
            return True
